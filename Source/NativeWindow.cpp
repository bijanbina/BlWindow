...
//				}
			...
			case WM_ERASEBKGND:
				...
				HDC device_context{BeginPaint(handle, &paint_struct)};
				FillRect(device_context, &paint_struct.rcPaint,
	...
	int device_pixel_ratio, const COLORREF& background_color, int x, int y)
{
    ...
    std::wstring ti = L"hey";

    handle = CreateWindowW(wc.lpszClassName, ti.c_str(),
        flags, x, y, width, height, 0, 0, h_instance, nullptr);

    if( !handle )
    {
        qDebug() << "module_handle" << h_instance
                 << "width" << width
                 << "height" << height
                 << "x" << x
                 << "y" << y
        ...
        throw std::runtime_error("CreateWindowA Failed");
    };
...
{
	minimum_width = width*device_pixel_ratio;
	minimum_height = height*device_pixel_ratio;
}
